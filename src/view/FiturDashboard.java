...
public class FiturDashboard extends javax.swing.JPanel {
    ...
    private JFreeChart createChart(DefaultCategoryDataset dataset) {
        ...
        StandardChartTheme theme = new StandardChartTheme("Modern");
        theme.setExtraLargeFont(new Font("SansSerif", Font.BOLD, 16));
        theme.setLargeFont(new Font("SansSerif", Font.BOLD, 12));
        theme.setRegularFont(new Font("SansSerif", Font.PLAIN, 10));
        theme.setSmallFont(new Font("SansSerif", Font.PLAIN, 9));
        ...
        AreaRenderer areaRenderer = new AreaRenderer() {
            ...
            public Paint getItemPaint(int row, int col) {
                return new GradientPaint(
                    0, 0, new Color(3, 78, 143, 100),   
                    0, 500, new Color(3, 78, 143, 10)    
                );
            }
        };
        areaRenderer.setEndType(AreaRendererEndType.LEVEL);

        // Configure rest of the chart
    ...
    private void updateChart(String filter) {
        try {
            ...
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Error updating chart: " + e.getMessage());
        }
    }
    ...
    private DefaultCategoryDataset createDataset(String filter) {
        ...
        try {
            // Modified query to count all sales in each month correctly
            String query = "SELECT MONTHNAME(tanggal_transaksi) AS bulan, " +
            ...
            while (rst.next()) {
                int jumlah_penjualan = rst.getInt("jumlah_penjualan");
                String bulan = rst.getString("bulan");
                dataset.addValue(jumlah_penjualan, "Penjualan", bulan);
            }

        } catch (SQLException e) {
            JOptionPane.showMessageDialog(this, "Error saat mengambil data dari database: " + e.getMessage(), 
    ...
    private void animateChartLine(DefaultCategoryDataset dataset, ChartPanel chartPanel) {
        javax.swing.Timer timer = new javax.swing.Timer(150, new ActionListener() {
            int currentIndex = 0;
            DefaultCategoryDataset animatedDataset = new DefaultCategoryDataset();
    ...
    private void startAutoRefresh(JFreeChart chart) {
        Timer refreshTimer = new Timer(30000, (ActionEvent e) -> { // Refresh every 30 seconds
            DefaultCategoryDataset newDataset = createDataset(null);
            chart.getCategoryPlot().setDataset(0, newDataset);
            chart.getCategoryPlot().setDataset(1, newDataset);
        });
        refreshTimer.start();
    }
}